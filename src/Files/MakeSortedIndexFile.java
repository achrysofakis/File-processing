...
public class MakeSortedIndexFile {
	public static void processArray(DataPage[] pdata,int DataPageSize) throws IOException {
		  //  byte[] zeroBytes = new byte[FIELD_SIZE];
			...
			for(j=0; j<(pages); j++){
				...
				for(i=0; i<(DataPageSize/8); i++){//We full our buffer and need to clear it after this loop(1 page loop)
			    	...
			    	WriteDataPageBuffer.putInt(pdata[sum].getPageNumber());		// Put  our key(4 bytes) into DataPage
			    	serialized_data_size += Integer.SIZE/8;  // Integer.SIZE is 32 bits or 4 bytes
			        //System.out.println("Total size of all serialized data in DataPage: " + serialized_data_size + " bytes (2 ints of 4 bytes each)");
			        sum=sum+1;
				}

				 // ================= Write to the file =====================================
		        RandomAccessFile MyFile = new RandomAccessFile ("TheIndexSortedFile", "rw");
		        MyFile.seek(pos);
